...
class SizeControl extends React.Component {
  ...
  render() {
    ...
    if (
      ...
      (this.props.min || this.props.max || this.props.step)
    ) {
      // We have some config

      const config = {
        min:
          this.props.layer.editable[this.props.name].min || this.props.min || 0,
        max:
          this.props.layer.editable[this.props.name].max ||
          this.props.max ||
          null,
        step:
          this.props.layer.editable[this.props.name].step ||
          this.props.step ||
          1
      };
      ...
      return (
        <div>
          <strong>
            ...
            <code>{this.props.layer[this.props.name]}</code>
          </strong>
          <input
            {...config}
